...
class BLLManagerTest {
    ...
    void getPhotosNumbersforOrder() {
        ...
        assertTrue(returnNumber > 0,
                "Return number for valid product should be greater than 0");
        assertEquals(0, invalidReturnNumber,
                "Return number for invalid product should be 0");
        assertEquals(0, bllManager.getPhotosNumbersforOrder(null),
                "Return number for null product should be 0");
    }
    ...
    void getPhotosForOrder() {
        ...
        assertNotNull(photos,
                "Photos list should not be null");
        assertFalse(photos.isEmpty(),
                "Photos list for valid order should not be empty");
        assertTrue(photos.stream().anyMatch(photo -> photo.getPhotoFile() != null && photo.getPhotoFile().length > 0),
                "Photos should contain a valid binary element");

        List<Photo> invalidPhotos = bllManager.getPhotosForOrder(invalidProductNoString);
        assertNotNull(invalidPhotos,
                "Photos list for invalid order should not be null");
        assertTrue(invalidPhotos.isEmpty(),
                "Photos list for invalid order should be empty");
    }
    ...
    void isDocumentExists() {
        ...
        assertFalse(notExists,
                "Document with product number " + invalidProductNoString + " should not exist");
    }
    ...
    void userOperations() {
        ...
        assertEquals(user.getFullName(), bllManager.getUserById(addedUser.getId()).getFullName(),
                "Updated full name should match");
    }
